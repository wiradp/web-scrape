"""
================================================================================
EXTRACTORS MODULE - Laptop Feature Extraction & Standardization
================================================================================

This module is responsible for extracting and standardizing laptop features
from unstructured product names (unstructured text). Each function is designed
to handle inconsistent data and produce standardized output.

OVERVIEW:
---------
The extractors.py module provides functions for extracting laptop information from
unstructured text, including:
  ...
  - RAM (Memory)
  - Storage (Penyimpanan)
  - Display (Ukuran layar)

MAIN FUNCTIONS:
-------------

1. BRAND & SERIES EXTRACTION:
   - get_brands()                  : Return a list of supported laptop brands
   - extract_brand()               : Extracting the brand from the product name
   - extract_series()              : Extracting product series/models

2. HARDWARE EXTRACTION:
   - extract_processor()           : Extracting CPU information
   - extract_gpu()                 : Extracting GPU information
   - extract_ram()                 : Extracting RAM capacity
   - extract_storage()             : Extracting Storage capacity
   - extract_display()             : Extracting screen size

3. STANDARDIZATION:
   - standardize_processor()       : Standardizing processor output
   - standardize_gpu()             : Standardizing GPU output

KEY FEATURES:
------------
✓ Robust Pattern Matching      : Using regex to handle text variations
✓ Fuzzy Matching               : Matching similar strings (typo handling)
✓ Brand Detection              : Brand detection based on position and context
✓ Model-Specific Logic         : Specific logic for each brand
✓ Error Handling               : Graceful handling for invalid data
✓ Standardized Output          : Consistent output for each category

USE:
-----------
  from extractors import extract_brand, extract_processor, extract_gpu
  
  brand = extract_brand("ASUS VivoBook 15 Intel Core i5", get_brands())
  processor = extract_processor("Intel Core i5-1235U")
  gpu = extract_gpu("NVIDIA GeForce RTX 3050")

IMPORTANT NOTES:
----------------
- All functions are designed for handling imperfect data.
- Output ‘Unknown’ or ‘Other’ indicates data that cannot be identified.
- This function is part of the ETL pipeline for the laptop marketplace.
- Each function receives input in the form of strings from scraped product data.
...
def get_brands():
    """
    Providing a list of popular laptop brands for feature extraction.
    """
    return [
...
def extract_brand(product_name, brand_list):
    ...
    for brand in brand_list:
        if re.search(r'\b' + re.escape(brand) + r'\b', product_text, re.IGNORECASE):
            logger.debug(f"Brand detected: {brand} from '{product_name}'")
            return brand
    
    logger.debug(f"Brand not found in '{product_name}', returning 'Other'")
    return 'Other'
...
def extract_processor(product_name):
    """
    Extract and standardize the Processor name from the product name.
    """
    ...
    for pattern, formatter in final_patterns:
        ...
        if match:
            ...
            if result and not re.search(r'(RAM|GB|SSD|HDD|VGA|RADEON|VEGA)\s*' + re.escape(match.group(0)), name_upper):
                logger.debug(f"Processor detected: {result} from '{product_name}'")
                return result
    
    logger.debug(f"Unknown processor in '{product_name}'")
    return 'Unknown Processor'

def standardize_processor(processor):
    """
    Standardize and group processors into more structured categories
    """
...
def extract_gpu(gpu_name):
    """
    Extracting and standardizing GPU names from product names.
    """
    ...
    for keyword, gpu_name in vendor_patterns:
        if keyword in name_upper:
            logger.debug(f"GPU detected: {gpu_name} from '{gpu_name}' input")
            return gpu_name
    
    logger.debug(f"Unknown GPU in '{gpu_name}'")
    return 'Unknown Graphics'

def standardize_gpu(gpu_name):
    """
    Standardize and group GPUs into more structured categories
    """
...
def extract_ram(ram_size):
    """
    Extracting and standardizing RAM size from product names.
    """
    ...
    for pattern, condition in fallback_patterns:
        ...
        if match and condition(match):
            ...
            if ram_size in [2, 4, 8, 16, 32, 64, 128]:
                logger.debug(f"RAM detected: {ram_size}GB from '{ram_size}' input")
                return f"{ram_size}GB"
    
    logger.debug(f"Unknown RAM in '{ram_size}'")
    return 'Unknown RAM'

def extract_storage(storage_size):
    """
    Extract and standardize storage specifications from product names.
    """
    ...
    result = f"{main_storage['capacity']}{main_storage['unit']}"
    logger.debug(f"Storage detected: {result} from '{storage_size}' input")
    return result

def extract_display(display_size):
    """
    Extract and standardize screen sizes from product names
    """
    ...
    if candidates:
        ...
        if size.endswith('.0'):
            size = size[:-2]
        result = f'{size}"'
        logger.debug(f"Display detected: {result} from '{display_size}' input")
        return result

    logger.debug(f"Unknown display in '{display_size}'")
    return 'Unknown'