...
def extract_series(product_name: str) -> str:
    ...
    if detected_brand == "asus":
        # Prioritize Advantage Edition models to ensure correct classification
        ...
        if "vivobook pro" in name_lower or re.search(r"\bv\d{4}", name_lower):
            return "Vivobook Pro"
        if "vivobook s" in name_lower or re.search(r"\bk\d{3}|um\d{3}", name_lower):
            return "Vivobook S"
        ...
        # --- Advantage Edition specific fix ---
        if "advantage edition" in name_lower:
        ...
        # NEW: Additional specific model code detection
        ...
        if re.search(r"fa617|fa507", product_name, re.IGNORECASE):
            return "TUF Gaming"
        if re.search(r"g513|g733", product_name, re.IGNORECASE):
            return "ROG Strix"

        # === AUTO-ADDED (SAFE PATCH) ===
        # Tangani kasus tambahan hasil evaluasi Unknown
...
def run_etl(db_path: str = 'data/database/laptop_data_raw.db', output_db_path: str = 'data/database/laptops.db'):
    """
    Jalankan seluruh pipeline feature engineering.
    Membaca data hasil scraping dari CSV, melakukan ekstraksi fitur,
    dan menyimpan hasilnya ke database SQLite (bukan CSV).
    """
    logger.info("=" * 80)
    logger.info("Starting the Feature Engineering process")
    logger.info(f"Input file: {db_path}")
    logger.info(f"Output database: {output_db_path}")

    # Validasi keberadaan file database input
    if not os.path.exists(db_path):
        logger.error(f"Input database not found: {db_path}")
        return

    try:
        # --- BACA DARI DATABASE INTEGRASI (products_raw) ---
        conn = sqlite3.connect(db_path)
        # Ambil data mentah dari tabel products_raw
        query = "SELECT id AS raw_id, product_name, price_raw FROM product_raw ORDER BY id DESC;"
    ...
    # Buat tabel products_enriched jika belum ada
    cursor.execute("""
    ...
    for _, row in df.iterrows():
        records.append((
             row['raw_id'], # <-- Gunakan raw_id dari database
            row['product_name'], # Simpan nama produk asli
            row.get('brand'),
            row.get('series'),
            row.get('processor_detail'),
            row.get('processor_category'),
            row.get('gpu'),
            row.get('gpu_category'),
            row.get('ram'),
            row.get('storage'),
            row.get('display'),
            row['price_raw'],
            row['price_in_millions'],
            datetime.now(), # processed_at
            datetime.now() # valid_from
        ))

    # Bulk insert ke products_enriched
    cursor.executemany("""
        INSERT INTO products_enriched (
            raw_id, raw_product_name, brand, series, processor_detail, processor_category,
            gpu, gpu_category, ram, storage, display, price_raw, price_in_millions, processed_at, valid_from
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, records)
    ...
    logger.info(f"✅ Feature engineering is complete. {len(records)} items saved to database: {output_db_path}")
...
if __name__ == "__main__":
    ...
    output_db_path = "data/database/laptops.db"               
    run_etl(input_db_path, output_db_path)
